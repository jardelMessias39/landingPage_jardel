            ...
            return (
              <div key={project.id} className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-12 items-stretch`}>
                ...
                <div className="w-full lg:w-1/2 flex flex-col justify-center space-y-6">
                  ...
                  {/* Technologies */}
                  <div className="pt-2">
                    <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Tecnologias Utilizadas</h4>
                  ...
                  <div className="flex items-center gap-4 pt-4">
                    {project.demo && (
                      <a href={project.demo} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition-colors">
                        Ver ao vivo <ArrowUpRight className="h-4 w-4" />
                      </a>
                    )}
                    {project.github && (
                      <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-2.5 rounded-full glass-panel text-white hover:bg-white/10 text-sm font-semibold transition-colors">
                        <GitBranch className="h-4 w-4" /> Repositório
                      </a>
                    )}
                    {(!project.demo && !project.github) && (
                      <div className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 text-slate-400 text-sm font-medium cursor-not-allowed">
